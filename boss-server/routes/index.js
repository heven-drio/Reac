...
/*
  a) path为: /register
  b) 请求方式为: POST
  c) 接收username 和 password 参数
  d) admin 是已注册用户
  e) 注册成功返回：{code:0,data:{_id:"abc",username:"xxx",passworld:"123"}}
...
router.post('/register',function(req,res){
  ...
  //处理: 判断用户是否存在,如果存在,返回错误提示信息，若不存在,保存
    //查询（根据username）
    UserModel.findOne({username},function(err,user){
      ...
      if(user){
        ...
        new UserModel({username,type,password:md5(password)}).save(function(error,user){
          ...
          //返回包含user的json数据
          const data = {username,type,_id:user.id} //响应数据中不要携带密码password
